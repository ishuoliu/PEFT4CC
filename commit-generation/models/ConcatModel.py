...
class RobertaClassifier(nn.Module):
    def __init__(self, args):
        ...
        self.cat_proj = nn.Linear(2 * args.hidden_size, args.hidden_size)
...
class ConcatModel(nn.Module):
    def __init__(self, basemodel, config, tokenizer, args):
        ...
        self.args = args
        self.concatlayer = RobertaClassifier(args)
    ...
    def forward(self, diff_ids, diff_mask, msg_ids, msg_mask, manual_features):
        if self.args.pretrained_model in ["codet5", "plbart", "plbart-large"]:
            encoder_output = self.encoder(input_ids=diff_ids, attention_mask=diff_mask)
            concat_encoder_output = self.concatlayer(encoder_output, manual_features)
            decoder_outputs = self.decoder(input_ids=msg_ids,
                                           attention_mask=msg_mask,
                                           encoder_hidden_states=concat_encoder_output,
                                           encoder_attention_mask=diff_mask)


        elif self.args.pretrained_model in ["codebert", "graphcodebert", "unixcoder"]:
            encoder_output = self.encoder(input_ids=diff_ids, attention_mask=diff_mask)
            concat_encoder_output = self.concatlayer(encoder_output, manual_features)
            decoder_outputs = self.decoder(tgt=msg_ids,
                                           memory=concat_encoder_output,
                                           tgt_mask=msg_mask,
                                           memory_mask=diff_mask)
    ...
    def generate_preds(self, diff_ids, diff_mask):
        top_preds = []
        if self.args.pretrained_model in ["codet5", "plbart", "plbart-large"]:
            preds = self.basemodel.generate(diff_ids, attention_mask=diff_mask)
