...
class Seq2Seq_uni(nn.Module):
    ...
    def __init__(self, encoder, decoder, config, args, beam_size=None, max_length=None, sos_id=None, eos_id=None):
        super(Seq2Seq_uni, self).__init__()
    ...
    def forward(self, source_ids, target_ids=None):
        ...
        if self.args.use_lora:
            encoder_output = self.encoder.base_model.model(source_ids, attention_mask=mask, use_cache=True)
        else:
        ...
        if self.args.use_lora:
            out = self.decoder.base_model.model(target_ids, attention_mask=mask,
    ...
    def generate(self, source_ids):
        ...
        for i in range(source_ids.shape[0]):
            ...
            for _ in range(self.max_length):
                ...
                mask = mask & ids[:, None, :].ne(1)
                if self.args.use_lora:
                    out = self.decoder.base_model.model(input_ids, attention_mask=mask, past_key_values=context).last_hidden_state
                else:
